main()
{
	if((isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="match")||getDvar("promod_mc_enable")==""||getDvarInt("promod_mc_enable")<1)return;
...
Run_Messages()
{
	...
	if(getDvar("promod_mc_delay")=="")setDvar("promod_mc_delay","20");
	generic_delay=20;
	if(getDvar("promod_mc_maxmessages")=="")setDvar("promod_mc_maxmessages",20);
	for(;
	;
	)
	{
...
Get_Next_Map()
{
	...
	i++)
	{
		...
		if(temparr2[i]=="gametype")
		{
			...
			x++)
			{
				if(temparr2[i+x]!=" ")gt=temparr2[i+x];
				break;
			}
		}
	...
	if(!isdefined(map))return undefined;
	if(!isdefined(gt))gt=getDvar("g_gametype");
