...
main()
{
	...
	{
		if(game["promod_in_timeout"])sb_text="timeout";
		else
		{
			if(game["promod_first_readyup_done"])sb_text="2nd_half";
			else sb_text="1st_half";
		}
	...
	setGameEndTime(0);
	if(game["promod_first_readyup_done"])thread periodAnnounce();
	...
	while(!level.ready_up_over)
	{
		...
		i++)
		{
			...
			if(player.oldready!=player.newready&&(!isDefined(player.inrecmenu)||!player.inrecmenu))
			{
				...
				if(player.ready)player.statusicon="compassping_friendlyfiring_mp";
		...
		if(all_players_ready)level.ready_up_over=true;
		wait 0.05;
	}
	...
	for(i=0;
	i<level.players.size;
	i++)level.players[i]ShowScoreBoard();
	game["state"]="postgame";
			...
			i++)
			{
				...
				else
				{
					if((!isDefined(player.soundplayed)||gettime()-20000>player.soundplayed)&&(!isDefined(player.timesplayed)||player.timesplayed<4)&&(!isDefined(player.inrecmenu)||!player.inrecmenu))
					{
						...
						if(isDefined(player.timesplayed))player.timesplayed++;
						else player.timesplayed=1;
					}
				}
			}
		}
...
updatePlayerHUDInterval()
{
	...
	while(!level.ready_up_over)
	{
		...
		i++)
		{
			...
			if(isDefined(player))
			{
				if(isDefined(player.ready)&&!isDefined(player.inrecmenu))player setclientdvar("self_ready",int(player.ready));
				if(isDefined(level.not_ready_count))player setclientdvar("waiting_on",level.not_ready_count);
			}
		}
	}
}
selfLoop()
{
	...
	while(!level.ready_up_over)
	{
		while(!isDefined(self.pers["team"])||self.pers["team"]=="none")wait 0.05;
		wait 0.05;
		if(self useButtonPressed())self.ready=!self.ready;
...
{
	self endon("disconnect");
	if(!game["promod_first_readyup_done"])self waittill("spawned_player");
	text="";
	if(!game["promod_first_readyup_done"])text="Pre-Match";
	else if(game["promod_in_timeout"])text="Timeout";
	else text="Half-Time";
	...
	if(game["promod_first_readyup_done"])
	{
		...
		if(game["promod_in_timeout"]&&(!isDefined(game["LAN_MODE"])||!game["LAN_MODE"]))text="Remaining";
		else text="Elapsed";
	...
	if(isDefined(self.periodtext))self.periodtext destroy();
	if(isDefined(self.halftimetext))self.halftimetext destroy();
}
onSpawn()
{
	self endon("disconnect");
...
periodAnnounce()
{
	...
	if(!game["promod_in_timeout"]||isDefined(game["LAN_MODE"])&&game["LAN_MODE"])level.halftimetimer setTimerUp(0);
	...
	level waittill("kill_ru_period");
	if(isDefined(level.halftimetimer))level.halftimetimer destroy();
}
moveOver()
{
	level endon("kill_ru_period");
	...
	{
		wait 2.6;
		if(isDefined(level.halftimetimer))level.halftimetimer.alpha=1;
		level.rup_txt_fx=false;
	}
	if(isDefined(self.halftimetext))self.halftimetext.alpha=1;
}