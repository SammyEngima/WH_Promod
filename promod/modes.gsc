...
validMode(mode)
{
	switch(mode)
	{
		case"comp_public":case"comp_public_hc":case"custom_public":case"comp_public_lan":case"comp_public_hc_lan":case"custom_public_lan":case"strat":case"match":case"knockout":return true;
	}
	keys=strtok(mode,"_");
	...
	i++)
	{
		switch(keys[i])
		{
			case"match":case"knockout":if(switches["match_knockout"])return false;
			switches["match_knockout"]=true;
			break;
			case"1v1":case"2v2":if(switches["1v1_2v2"])return false;
			switches["1v1_2v2"]=true;
			break;
			case"lan":case"pb":if(switches["lan_pb"])return false;
			switches["lan_pb"]=true;
			break;
			case"knife":if(switches["scores_done"])return false;
			case"hc":if(switches[keys[i]+"_done"])return false;
			...
			else if((isSubStr(keys[i],":"))&&strtok(keys[i],":").size==2&&int(strtok(keys[i],":")[0])>=0&&int(strtok(keys[i],":")[1])>=0&&!switches["scores_done"]&&!switches["knife_done"])switches["scores_done"]=true;
			else return false;
			break;
		}
	}
...
setMode(mode)
{
	...
	if(game["PROMOD_MATCH_MODE"]=="")
	{
		...
		i++)
		{
			switch(exploded[i])
			{
				...
				break;
				default:if(isSubStr(exploded[i],"mr"))mr_rating=int(strtok(exploded[i],"mr")[0]);
	...
	if(knockout_mode)game["PROMOD_MODE_HUD"]+="^4Knockout";
	else if(game["PROMOD_MATCH_MODE"]=="match")game["PROMOD_MODE_HUD"]+="^4Match";
	if(game["PROMOD_KNIFEROUND"]&&game["PROMOD_MATCH_MODE"]=="match"&&level.gametype=="sd")game["PROMOD_MODE_HUD"]+=" ^5Knife";
	if(game["LAN_MODE"])
	{
		setDvar("g_antilag",0);
		setDvar("g_smoothClients",0);
		game["PROMOD_MODE_HUD"]+=" ^4LAN";
		if(knockout_mode)game["PROMOD_STRATTIME"]=10;
	}
	if(game["HARDCORE_MODE"])
	{
		if(game["PROMOD_MATCH_MODE"]=="match")game["PROMOD_MODE_HUD"]+=" ^6HC";
