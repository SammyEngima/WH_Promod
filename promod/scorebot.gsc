main()
{
	if(getDvar("promod_enable_scorebot")=="")setDvar("promod_enable_scorebot",0);
...
updateTimer()
{
	if(!isDefined(game["prevtime"]))game["prevtime"]=gettime();
	for(;
	;
	)
	{
...
actionTicker()
{
	while(!isDefined(game["promod_scorebot_attack_ticker_buffer"])||!isDefined(game["promod_scorebot_defence_ticker_buffer"]))wait 0.05;
	...
	setDvar("__promod_version","EPT Promod by: Delfinpuki",true);
	for(;
	;
	)
	{
