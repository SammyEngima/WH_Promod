...
init()
{
	...
	level thread domi\adminpanel::init();
	level thread domi\fullbright::init();
...
Callback_StartGameType()
{
	...
	thread maps\mp\gametypes\_admin_menu_response::init();
	//thread promod\welcome::dlc_msg_go();
	thread maps\mp\gametypes\_bots::addTestClients();
...
Callback_PlayerConnect()
{
	...
	thread maps\mp\gametypes\_admin_menu_response::onMenuResponse();
...
Callback_PlayerKilled(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration)
{
	...
	if (!isDefined(game["state"]) || game["state"] != "playing")
	{
		level thread promod\_finalkillcam::startFinalKillcam( self getKillcamWatch( attacker, eInflictor ), self getEntityNumber(), killcamentity, sWeapon, self.deathTime, 0, psOffsetTime, attacker );
		return;
	}
