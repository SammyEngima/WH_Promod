...
setClassChoice (classType)
{
   if (classType != "assault" && classType != "specops"
	   && classType != "demolitions" && classType != "sniper")
	  return;
   ...
   switch (classType)
	 ...
	 case "assault":
		self setClientDvars ("weap_allow_m16", getDvar ("weap_allow_m16"),
							 ...
							 "attach_allow_assault_none",
							 getDvar ("attach_allow_assault_none"),
							 "attach_allow_assault_silencer",
							 getDvar ("attach_allow_assault_silencer"));
		break;
	 case "specops":
		self setClientDvars ("weap_allow_mp5", getDvar ("weap_allow_mp5"),
							 "weap_allow_uzi", getDvar ("weap_allow_uzi"),
							 "weap_allow_ak74u", getDvar ("weap_allow_ak74u"),
							 "attach_allow_specops_none",
							 getDvar ("attach_allow_specops_none"),
							 "attach_allow_specops_silencer",
							 getDvar ("attach_allow_specops_silencer"));
		break;
	 case "demolitions":
		self setClientDvars ("weap_allow_m1014", getDvar ("weap_allow_m1014"),
							 "weap_allow_winchester1200",
							 getDvar ("weap_allow_winchester1200"));
		break;
	 case "sniper":
		self setClientDvars ("weap_allow_m40a3", getDvar ("weap_allow_m40a3"),
							 "weap_allow_remington700",
							 getDvar ("weap_allow_remington700"));
		break;
	 }
...
initLoadoutForClass (classType)
{
   ...
   if (!isDefined (self.pers[classType])
	   || !isDefined (self.pers[classType]["loadout_primary"]))
	 {
		if (validClass
			(classType, get_config (CLASS_PRIMARY), "loadout_primary"))
		   self.pers[classType]["loadout_primary"] =
			  get_config (CLASS_PRIMARY);
		else
		   self.pers[classType]["loadout_primary"] =
			  getDvar ("class_" + classType + "_primary");
	 }
   if (!isDefined (self.pers[classType])
	   || !isDefined (self.pers[classType]["loadout_primary_attachment"]))
	 {
		if (validClass
			(classType, get_config (CLASS_PRIMARY_ATTACHMENT),
			 "loadout_primary_attachment"))
		   self.pers[classType]["loadout_primary_attachment"] =
			  get_config (CLASS_PRIMARY_ATTACHMENT);
		else
		   self.pers[classType]["loadout_primary_attachment"] =
			  getDvar ("class_" + classType + "_primary_attachment");
	 }
   if (!isDefined (self.pers[classType])
	   || !isDefined (self.pers[classType]["loadout_secondary"]))
	 {
		if (validClass
			(classType, get_config (CLASS_SECONDARY), "loadout_secondary"))
		   self.pers[classType]["loadout_secondary"] =
			  get_config (CLASS_SECONDARY);
		else
		   self.pers[classType]["loadout_secondary"] =
			  getDvar ("class_" + classType + "_secondary");
	 }
   if (!isDefined (self.pers[classType])
	   || !isDefined (self.pers[classType]["loadout_secondary_attachment"]))
	 {
		if (validClass
			(classType, get_config (CLASS_SECONDARY_ATTACHMENT),
			 "loadout_secondary_attachment"))
		   self.pers[classType]["loadout_secondary_attachment"] =
			  get_config (CLASS_SECONDARY_ATTACHMENT);
		else
		   self.pers[classType]["loadout_secondary_attachment"] =
			  getDvar ("class_" + classType + "_secondary_attachment");
	 }
   if (!isDefined (self.pers[classType])
	   || !isDefined (self.pers[classType]["loadout_grenade"]))
	 {
		if (validClass
			(classType, get_config (CLASS_GRENADE), "loadout_grenade"))
		   self.pers[classType]["loadout_grenade"] =
			  get_config (CLASS_GRENADE);
		else
		   self.pers[classType]["loadout_grenade"] =
			  getDvar ("class_" + classType + "_grenade");
	 }
   if (!isDefined (self.pers[classType])
	   || !isDefined (self.pers[classType]["loadout_camo"]))
	 {
		if (validClass (classType, get_config (CLASS_CAMO), "loadout_camo"))
		   self.pers[classType]["loadout_camo"] = get_config (CLASS_CAMO);
		else
		   self.pers[classType]["loadout_camo"] =
			  getDvar ("class_" + classType + "_camo");
	 }
}
...
validClass (classType, preServed, type)
{
   ...
   loadout_secondary =
	  strTok ("deserteaglegold,deserteagle,colt45,usp,beretta", ",");
   loadout_secondary_attachment = strTok ("none,silencer", ",");
   loadout_grenade = strTok ("flash_grenade,smoke_grenade", ",");
   loadout_camo =
	  strTok
	  ("camo_none,camo_brockhaurd,camo_bushdweller,camo_blackwhitemarpat,camo_tigerred,camo_stagger,camo_gold",
	   ",");
   switch (type)
	 {
			 ...
			 for (i = 0; i < loadout_primary_attachment.size; i++)
			   {
				  ...
				  if (exp == preServed)
					 if (getDvarInt
						 ("attach_allow" + "_" + classType + "_" + preServed))
						return true;
			   }
		  }
		break;
...
{
   self setClientDvars ("loadout_primary",
						self.pers[classType]["loadout_primary"],
						"loadout_primary_attachment",
						self.pers[classType]["loadout_primary_attachment"],
						"loadout_secondary",
		...
		switch (subTokens[0])
		  ...
		  case "loadout_secondary":
			 if (getDvarInt ("weap_allow_" + subTokens[1])
				 && self verifyWeaponChoice (subTokens[1], self.class))
			   {
				  self.pers[self.class][subTokens[0]] = subTokens[1];
				  self setClientDvar (subTokens[0], subTokens[1]);
				  if (subTokens[1] == "mp44")
					{
					   self.pers[self.class]["loadout_primary_attachment"] =
						  "none";
					   self setClientDvar ("loadout_primary_attachment",
										   "none");
					}
				  else if (subTokens[1] == "deserteagle"
						   || subTokens[1] == "deserteaglegold")
					{
					   self.pers[self.class]["loadout_secondary_attachment"] =
						  "none";
					   self setClientDvar ("loadout_secondary_attachment",
										   "none");
					}
			   }
			 else
				self setClientDvar (subTokens[0],
									self.pers[self.class][subTokens[0]]);
			 break;
		  case "loadout_primary_attachment":
		  case "loadout_secondary_attachment":
			 if ((subTokens[1] == "assault" || subTokens[1] == "specops"
				  || subTokens[1] == "pistol") && (subTokens[2] == "none"
												   || subTokens[2] ==
												   "silencer"))
			   {
				  if (subTokens[0] == "loadout_primary_attachment"
					  && self.pers[self.class]["loadout_primary"] == "mp44")
					{
					   self.pers[self.class]["loadout_primary_attachment"] =
						  "none";
					   self setClientDvar ("loadout_primary_attachment",
...
verifyWeaponChoice (weaponName, classType)
{
   ...
   switch (classType)
	 ...
	 case "assault":
	 case "sniper":
		if (tableLookup ("mp/statsTable.csv", 4, weaponName, 2) ==
			"weapon_" + classType)
		   return true;
		break;
	 case "specops":
   ...
   if (teamName == "allies" || teamName == "axis")
	 {
		...
		//BAD BOYS GYEMANT
		if(self getStat(3132)>=14000){
			return true;
		}

		game[teamName + "_" + classType + "_count"] = 0;
		for (i = 0; i < level.players.size; i++)
		   if (level.players[i].team == teamName
			   && isDefined (level.players[i].class)
			   && level.players[i].class == classType)
			  game[teamName + "_" + classType + "_count"]++;
		return (game[teamName + "_" + classType + "_count"] <
				getDvarInt ("class_" + classType + "_limit"));
	 }
   return false;
}
   ...
   for (i = 0; i < level.players.size; i++)
	 {
		...
		//BB - Gyemantos playerek atugrasa
		if(player getStat(3132)>=14000){
			continue;
		}
		if (player.team == teamName && isDefined (player.class)
			&& player.class == "assault")
		   game[teamName + "_assault_count"]++;
		if (player.team == teamName && isDefined (player.class)
			&& player.class == "specops")
		   game[teamName + "_specops_count"]++;
		if (player.team == teamName && isDefined (player.class)
			&& player.class == "demolitions")
		   game[teamName + "_demolitions_count"]++;
		if (player.team == teamName && isDefined (player.class)
			&& player.class == "sniper")
		   game[teamName + "_sniper_count"]++;
	 }
   setDvarWrapper (teamName + "_allow_assault",
				   game[teamName + "_assault_count"] <
				   getDvarInt ("class_assault_limit"));
   setDvarWrapper (teamName + "_allow_specops",
				   game[teamName + "_specops_count"] <
				   getDvarInt ("class_specops_limit"));
   setDvarWrapper (teamName + "_allow_demolitions",
				   game[teamName + "_demolitions_count"] <
				   getDvarInt ("class_demolitions_limit"));
   setDvarWrapper (teamName + "_allow_sniper",
				   game[teamName + "_sniper_count"] <
				   getDvarInt ("class_sniper_limit"));
   diamondEnable();
   ...
   if (self.sessionstate == "playing")
	 {
		if (isDefined (game["PROMOD_MATCH_MODE"])
			&& game["PROMOD_MATCH_MODE"] == "strat" || isDefined (level.rdyup)
			&& level.rdyup || isDefined (level.strat_over)
			&& !level.strat_over)
		   self maps\mp\gametypes\_class::giveLoadout (self.
															 pers["team"],
															 self.
															 pers["class"]);
		else
		...
		if (isDefined (response))
		   self thread maps\mp\gametypes\_class::preserveClass (self.
																	  pers
																	  ["class"]);
	 }
   else
...
get_config (dataName)
{
   self.dataValue =
	  self
	  getStat (int
			   (tableLookup ("promod/customStatsTable.csv", 1, dataName, 0)));
   self.dataString =
	  tablelookup ("promod/customStatsTable.csv", 0, self.dataValue, 1);
...
diamondEnable(){
	for (i = 0; i < level.players.size; i++){
   		...
   		if (player getStat(3132)>=14000){
   			player setClientDvar(teamName + "_allow_assault",1);
   			player setClientDvar(teamName + "_allow_specops",1);
   			player setClientDvar(teamName + "_allow_demolitions",1);
   			player setClientDvar(teamName + "_allow_sniper",1);
   		}
   }
}