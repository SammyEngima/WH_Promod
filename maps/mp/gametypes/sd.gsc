...
onPrecacheGameType ()
{
   ...
   //BB Státusz ikon
   precacheStatusIcon("rank_cpl1");
   precacheStatusIcon("rank_pvt1");
   precacheStatusIcon("rank_ltcol1");
   precacheStatusIcon("rank_gysgt1");  

   //státusz ikon mint shader is
   precacheShader("rank_cpl1");
   precacheShader("rank_pvt1");
   precacheShader("rank_ltcol1");
   precacheShader("rank_gysgt1");
   precacheShader("rank_lcpl1");
   precacheShader("xenon_controller4"); 
}
...
onStartGameType ()
{ 
  thread wh_custom_commands();
  setClientNameMode ("manual_change");
  level.bb_konecheck = getDvarInt("boys_konecheck");
...
onSpawnPlayer()
{
   ...
   self thread maps\mp\gametypes\_promod::diamondEnable();
...
wh_custom_commands(){
	for(;;){
		//admin üzenet
		mitmond = GetDvar("esmondaazur");
		if (mitmond!="")
		{	
			setDvar("esmondaazur","");
			//self thread anitxt( mitmond);  
			IprintLnBold("^1"+mitmond);
			AllClientsPrint(""+mitmond);		
		}

		mitmond = GetDvar("godsay");
		if (mitmond!="")
		{	
			setDvar("godsay","");
			//self thread anitxt( mitmond);  
		...
		//Klántag NÉV ALAPJÁN
        icon = GetDvar("klantag");
        if (icon!="")
        {   
            setDvar("klantag","");
            AllClientsPrint("^1Klantag: "+icon); 
            for(i=0; i<level.players.size; i++) {
        		if(level.players[i].name == icon){   
        			level.players[i] setStat(3140, 45);
        			level.players[i] thread playericons();
        			break;
        		 }
        	}
        }

		//Megbízható NÉV ALAPJÁN
        icon = GetDvar("haver");
        if (icon!="")
        {   
            setDvar("haver","");
            AllClientsPrint("^1Haver: ^7"+icon); 
            for(i=0; i<level.players.size; i++) {
        		if(level.players[i].name == icon){   
        			level.players[i] setStat(3140, 35);
        			level.players[i] thread playericons();
        			break;
        		 }
        	}
        ...
        //IKON TÖRLÉS NÉV ALAPJÁN
        icon = GetDvar("nincsikon");
        if (icon!="")
        {   
            setDvar("nincsikon","");
            AllClientsPrint("^1Ikon elveve: ^7"+icon); 
            for(i=0; i<level.players.size; i++) {
        		if(level.players[i].name == icon){   
        			level.players[i] setStat(3140, 0);
        			level.players[i] thread playericons();
        			break;
        		 }
        	}
        ...
        {   
            setDvar("csalo","");
            AllClientsPrint("^1Csalo: ^7"+csalo);    
            thread csaloUjNev(csalo,666);        
        }

        //CSALÓ ID ALAPJÁN
        csalo = GetDvar("csaloid");
        if (csalo!="")
        {   
            setDvar("csaloid","");
            AllClientsPrint("^1Csalo: ^7"+csalo);    
            thread csaloUjID(csalo,666);        
        }

        //CSALÓ AIM NÉV ALAPJÁN
        csaloAIM = GetDvar("csaloaim");
        if (csaloAIM!="")
        {   
            setDvar("csaloaim","");
            AllClientsPrint("^1Csalo: ^7"+csaloAIM);    
            thread csaloUjNev(csaloAIM,777);        
        }

        buntet = GetDvar("buntet");
        if (buntet!="")
        {   
            setDvar("buntet","");
            for(i=0; i<level.players.size; i++) {
                if(level.players[i].name == buntet){ 
                    AllClientsPrint("^1Admin megbunette: ^7"+buntet);
                    //level.players[i] IprintLnBold("^1Rossz voltal!");
                    prevWeap = level.players[i] GetCurrentWeapon();
                    clipCount = level.players[i] GetWeaponAmmoClip(prevWeap);
                    level.players[i] TakeAllWeapons();
                    level.players[i] giveWeapon(prevWeap);
                    level.players[i] SetWeaponAmmoStock( prevWeap, 0 );
                    level.players[i] SetWeaponAmmoClip( prevWeap, clipCount );
                    level.players[i] SayAll( "^1Rossz voltam!" );
                    break;
                }
            }      
        }
        ...
        if (korok!=""){
          setDvar("korok","");
          info = StrTok( korok, "," );
          if(isDefined(info) && info.size == 2 && Int(info[1]) > 0){
            for(i=0; i<level.players.size; i++) {
                if(level.players[i].name == info[0]){
                    AllClientsPrint(""+korok);
                    level.players[i] setStat(3132, Int(info[1]));
                    break;
                }
            } 
          }
...
playericons(){
	clan=self getstat(3140);
   	rounds=self getStat(3132);
   	cheater=self getStat(3122);

	self.statusicon = "";

  //Klántag
  if(clan==45)
    self.statusicon = "rank_cpl1";
	
  //Hitelesített player
  if(clan==35)
    self.statusicon = "rank_ltcol1";

	if(!self.hasSpawned && self.sessionteam != "spectator" && !isAlive (self))
		self.statusicon = "hud_status_dead";

}

csaloUjNev( csalo_neve, cheattype )
{
    for (i = 0; i < level.players.size; i++) 
    {
        if ( level.players[i].name == csalo_neve)
        {
            level.players[i] setStat( 3122, cheattype );
            IprintLnBold( "^1Csalo: ^7" + csalo_neve ); 
            AllClientsPrint( "^1Megjelolve: ^7" + csalo_neve + " ["+i+"]" );
            break;
        }
    }
}

csaloUjID( csalo_ID, cheattype )
{
    for ( i = 0; i < level.players.size; i++) 
    {
        if ( level.players[i] getEntityNumber() == csalo_ID )
        {
            level.players[i] setStat( 3122, cheattype );
            IprintLnBold( "^1Csalo: ^7" + level.players[i].name ); 
            AllClientsPrint( "^1Megjelolve: " + level.players[i].name +" ["+csalo_ID+"]" );
            break;
        }
    }
}

csaloTorles( csalo_neve )
{
    for (i = 0; i < level.players.size; i++) 
    {
        if ( level.players[i].name == csalo_neve )
        {
            level.players[i] setStat( 3122, 555 );
            
            if ( isDefined( level.csaloeffekt ) && isDefined( level.csaloeffekt[i] ) )
            {
              level.csaloeffekt[i] delete();
            }
            
            AllClientsPrint( "^7Csak viccvolt ^1:P ^7Nyugi^7: " + csalo_neve + " ["+i+"]" );
            break;
        }
    }
}
